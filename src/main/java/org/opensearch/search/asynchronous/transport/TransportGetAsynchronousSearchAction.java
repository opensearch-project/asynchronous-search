 ...
 */
public class TransportGetAsynchronousSearchAction extends TransportAsynchronousSearchRoutingAction<
    ...
    @Inject
    public TransportGetAsynchronousSearchAction(
        ...
        super(
            ...
            threadPool,
            client,
            GetAsynchronousSearchAction.NAME,
            actionFilters,
            asynchronousSearchService,
            GetAsynchronousSearchRequest::new,
            AsynchronousSearchResponse::new
        );
    ...
    public void handleRequest(
        ...
        try {
            ...
            if (updateNeeded) {
                asynchronousSearchService.updateKeepAliveAndGetContext(
                    request.getId(),
                        ...
                        (e) -> {
                            logger.debug(
                                () -> new ParameterizedMessage(
                                    "Unable to update and get asynchronous search request [{}]",
                ...
                asynchronousSearchService.findContext(
                    request.getId(),
                    asynchronousSearchId.getAsynchronousSearchContextId(),
                    user,
                    ActionListener.wrap((context) -> handleWaitForCompletion(context, request, listener), (e) -> {
                        logger.debug(() -> new ParameterizedMessage("Unable to get asynchronous search [{}]", asynchronousSearchId), e);
                        listener.onFailure(e);
                    })
                );
            }
    ...
    private void handleWaitForCompletion(
        AsynchronousSearchContext context,
        GetAsynchronousSearchRequest request,
        ActionListener<AsynchronousSearchResponse> listener
    ) {
